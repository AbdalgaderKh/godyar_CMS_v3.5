...
if (is_file($functionsFile) === true) {
    require_once $functionsFile;
}

$bootstrap = __DIR__ . '/../includes/bootstrap.php';
if (gdy_is_file($bootstrap) === true) {
...
if (!function_exists('gdy_admin_normalize_url_value')) {
    function gdy_admin_normalize_url_value(string $value): string
    {
        ...
        $script = str_replace('\\', '/', gdy_server_var('SCRIPT_NAME', '/admin/index.php'));
...
try {
    $script = gdy_server_var('SCRIPT_NAME', '');
    $isApi = (stripos($script, '/admin/api/') !== false);

    if (
        !$isApi &&
        (gdy_request_method() === 'POST')
    ) {
    ...
    $back = gdy_server_var('HTTP_REFERER', $loginUrl);
    if (headers_sent() === false) {
        gdy_header('Location: ' . $back);
    }
    exit;
}

$__i18n = __DIR__ . '/i18n.php';
if (gdy_is_file($__i18n) === true) {
    require_once $__i18n;
}
...
if (gdy_is_file($authFile) === true) {
    require_once $authFile;
}

$__auditDb = __DIR__ . '/includes/audit_db.php';
if (gdy_is_file($__auditDb) === true) {
...
try {
    ...
    if (!$loggedIn) {
        if (function_exists('gdy_security_log')) {
            gdy_security_log('admin_unauthorized', ['path' => gdy_server_var('REQUEST_URI', '')]);
        }
        ...
        if ($pdo instanceof PDO) {
            ...
            if (!isset($_SESSION['session_version']) || !is_numeric($_SESSION['session_version'])) {
                ...
                if ($sessSv !== $dbSv) {
                    ...
                    if (headers_sent() === false) {
                        gdy_header('Location: ' . $loginUrl . '?msg=session_expired');
                    }
                    exit;
                }
            }
    ...
    function generate_csrf_token(): string
    {
        ...
        if (empty($_SESSION['_csrf_token'])) {
            try {
                ...
                $_SESSION['_csrf_token'] = hash('sha256', uniqid('csrf_', true));
            }
        }
...
if (!function_exists('verify_csrf')) {
    function verify_csrf(string $fieldName = 'csrf_token'): bool
    {
        ...
        $sent = gdy_post_var($fieldName, '');
        if ($sent === '') {
            $sent = gdy_server_var('HTTP_X_CSRF_TOKEN', '');
        }

        if (!verify_csrf_token($sent)) {
            http_response_code(400);

            $accept = gdy_server_var('HTTP_ACCEPT', '');
            $ctype  = gdy_server_var('CONTENT_TYPE', '');
