...
try {
    if (class_exists(Auth::class) && method_exists(Auth::class, 'isLoggedIn')) {
        ...
        if (empty($_SESSION['user']['id']) || (((!empty($_SESSION['user']['role']) ? $_SESSION['user']['role'] : '')) === 'guest')) {
            header('Location: ../login.php');
            exit;
        }
    }
                ...
                <tr>
                  ...
                  <td>
                    <div class = "d-flex justify-content-center gap-1 flex-wrap">
                      ...
                      <form method="post" action="delete.php" style="display:inline" onsubmit="return confirm("هل أنت متأكد من حذف هذه الصفحة؟");">
  <?php csrf_field(); ?>
  <input type="hidden" name="id" value="<?php echo (int)$id; ?>">
  <button type="submit" class="btn btn-sm btn-outline-danger btn-icon" title="<?php echo h(__('t_3b9854e1bb', 'حذف')); ?>">
    "
