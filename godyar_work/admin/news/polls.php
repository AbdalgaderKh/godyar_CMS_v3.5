    ...
    if (($checkCsrf === false)($csrf)) {
        ...
        try {
            ...
            if ($action === 'save' && $pollId > 0) {
                ...
                if ($isWriter) {
                    $st = $pdo->prepare('SELECT 1 FROM news_polls p JOIN news n ON n.id=p.news_id WHERE p.id=:p AND n.{$newsOwnerCol}=:u LIMIT 1');
...
$editVotes = 0;
if ($editId > 0) {
    $sql = "SELECT p.*, n.title AS news_title, n.{$newsOwnerCol} AS news_author_id
            FROM news_polls p
