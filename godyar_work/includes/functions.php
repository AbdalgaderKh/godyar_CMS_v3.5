...
if (function_exists('gdy_server_var') === false) {
    function gdy_server_var(string $key, string $default = ''): string {
        $value = filter_input(INPUT_SERVER, $key, FILTER_UNSAFE_RAW, FILTER_NULL_ON_FAILURE);
        if ($value === null) {
            return $default;
        }
        $value = gdy_unslash($value);
        return is_scalar($value) ? (string)$value : $default;
    }
}

if (function_exists('gdy_post_var') === false) {
    function gdy_post_var(string $key, string $default = ''): string {
...
if (function_exists('gdy_request_method') === false) {
    function gdy_request_method(): string {
        $method = strtoupper(gdy_server_var('REQUEST_METHOD', 'GET'));
        return $method !== '' ? $method : 'GET';
    }
}

if (function_exists('gdy_header') === false) {
    function gdy_header(string $value, bool $replace = true, ?int $responseCode = null): void {
        ...
        header($value, $replace, $responseCode);
    }
}

if (function_exists('gdy_is_dir') === false) {
    function gdy_is_dir(string $path): bool {
        return $path !== '' && is_dir($path) === true;
    }
}

if (function_exists('gdy_is_file') === false) {
    function gdy_is_file(string $path): bool {
        return $path !== '' && is_file($path) === true;
    }
}

if (function_exists('gdy_dirname') === false) {
    function gdy_dirname(string $path, int $levels = 1): string {
        $out = $path;
        for ($i = 0; $i < $levels; $i++) {
            $out = dirname($out);
        }
...
if (function_exists('gdy_uploaded_file') === false) {
    function gdy_uploaded_file(string $key): array {
        if (isset($_FILES[$key]) !== true || is_array($_FILES[$key]) !== true) {
            return [];
        }
        $file = $_FILES[$key];
        return [
            'name' => isset($file['name']) && is_scalar($file['name']) ? (string)gdy_unslash($file['name']) : '',
            'type' => isset($file['type']) && is_scalar($file['type']) ? (string)gdy_unslash($file['type']) : '',
            'tmp_name' => isset($file['tmp_name']) && is_scalar($file['tmp_name']) ? (string)gdy_unslash($file['tmp_name']) : '',
