...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && !empty($_POST['do']) && !empty($_POST['id'])) {
    $id = (int)$_POST['id'];
    $do = (string)$_POST['do'];
    ...
    if (!verify_csrf_token($csrf)) {
        if (function_exists('gdy_security_log')) { gdy_security_log('csrf_failed', ['area'=>'admin_comments','do'=>$do,'id'=>$id]); }
        http_response_code(400);
        $actionMsg = '<div class="alert alert-danger">طلب غير صالح (CSRF).</div>';
        $do = '';
    }
    ...
    if ($id > 0 && $do !== '') {
        ...
        if ($comment) {
            ...
            if ($do === 'approve') {
                $pdo->prepare("UPDATE news_comments SET status='approved' WHERE id=:id")->execute([':id' => $id]);
                $actionMsg = __('t_97ae8b25c7', 'تم قبول التعليق.');
            } elseif ($do === 'reject') {
                $pdo->prepare("UPDATE news_comments SET status='rejected' WHERE id=:id")->execute([':id' => $id]);
...
<title><?php echo h(__('t_08a0cc810c', 'إدارة التعليقات')); ?></title>
  <meta name = "viewport" content = "width=device-width, initial-scale=1">
    <link rel = "stylesheet" href="<?= h(asset_url('assets/vendor/bootstrap/css/bootstrap.rtl.min.css')) ?>">
</head>
<body class = "bg-light">
        ...
        <tr>
          ...
          <td>
            <form method="post" style="display:inline-block;margin:0">
              ...
              <button class="btn btn-sm btn-outline-success" type="submit"><?php echo h(__('t_d7e02c9230', 'قبول')); ?></button>
            </form>
            <form method="post" style="display:inline-block;margin:0">
              <input type="hidden" name="csrf_token" value="<?php echo h(generate_csrf_token()); ?>">
              <input type="hidden" name="id" value="<?php echo (int)$c['id']; ?>">
              <input type="hidden" name="do" value="reject">
              <button class="btn btn-sm btn-outline-secondary" type="submit"><?php echo h(__('t_b7dee9747a', 'رفض')); ?></button>
            </form>
            <form method="post" style="display:inline-block;margin:0" onsubmit="return confirm('حذف نهائي؟');">
              <input type="hidden" name="csrf_token" value="<?php echo h(generate_csrf_token()); ?>">
              <input type="hidden" name="id" value="<?php echo (int)$c['id']; ?>">
              <input type="hidden" name="do" value="delete">
              <button class="btn btn-sm btn-outline-danger" type="submit"><?php echo h(__('t_3b9854e1bb', 'حذف')); ?></button>
            </form>
          </td>
        </tr>
      <?php endforeach; ?>
