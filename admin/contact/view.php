...
<script nonce="<?= h($cspNonce) ?>">
  document.addEventListener("DOMContentLoaded", function () {
    const tCopied = "تم النسخ";
    const tCopyEmail = "نسخ البريد";
    const tCopyMessage = "نسخ نص الرسالة";

    
    const copyEmailBtn = document.getElementById("copyEmailBtn");
    const emailEl = document.getElementById("contactEmail");
    if (copyEmailBtn && emailEl && navigator.clipboard) {
      copyEmailBtn.addEventListener("click", function () {
        const email = (emailEl.textContent || "").trim();
        if (!email) return;
        const orig = copyEmailBtn.getAttribute("data-orig") || copyEmailBtn.innerHTML;
        copyEmailBtn.setAttribute("data-orig", orig);
        navigator.clipboard.writeText(email).then(function () {
          copyEmailBtn.innerHTML = '<svg class="gdy-icon me-1" aria-hidden="true" focusable="false"><use href="#copy"></use></svg> ' + tCopied;
          setTimeout(function () { copyEmailBtn.innerHTML = orig; }, 1500);
        });
      });
    }
    ...
    if (copyMessageBtn && messageContent && navigator.clipboard) {
      copyMessageBtn.addEventListener("click", function () {
        const text = (messageContent.innerText || messageContent.textContent || "").trim();
        if (!text) return;
        const orig = copyMessageBtn.getAttribute("data-orig") || copyMessageBtn.innerHTML;
        copyMessageBtn.setAttribute("data-orig", orig);
        navigator.clipboard.writeText(text).then(function () {
          copyMessageBtn.innerHTML = '<svg class="gdy-icon me-1" aria-hidden="true" focusable="false"><use href="#copy"></use></svg> ' + tCopied;
          setTimeout(function () { copyMessageBtn.innerHTML = orig; }, 1500);
        });
      });
    }
  });
