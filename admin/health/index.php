...
if ($pdo instanceof PDO) {
    try {
        ...
        try {
            $checks['db_name'] = (string)$pdo->query("SELECT DATABASE()")->fetchColumn();
            $checks['news_total'] = (int)$pdo->query("SELECT COUNT(*) FROM news")->fetchColumn();
            $checks['news_cat1'] = (int)$pdo->query("SELECT COUNT(*) FROM news WHERE category_id=1")->fetchColumn();
            $checks['has_news_lang'] = (bool)($pdo->query("SHOW COLUMNS FROM news LIKE 'lang'")->fetchColumn());
        } catch (\Throwable $e) {
          ...
          <div class = "card-body">
            ...
            <table class = "table table-sm table-borderless table-health mb-1">
              ...
              <tr>
                ...
                <td><?php echo h((string)($checks['db_name'] ?? '-')); ?></td>
              </tr>
              <tr>
                <th>إجمالي الأخبار</th>
                <td><?php echo h((string)($checks['news_total'] ?? '-')); ?></td>
              </tr>
              <tr>
                <th>أخبار القسم 
                <td><?php echo h((string)($checks['news_cat1'] ?? '-')); ?></td>
              </tr>
              <tr>
                <th>عمود news.lang</th>
                <td><?php echo !empty($checks['has_news_lang']) ? 'موجود' : 'غير موجود'; ?></td>
              </tr>
            </table>
