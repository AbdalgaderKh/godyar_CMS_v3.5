...
function backup_file(string $file, string $root): void {
    ...
    $backupDir = $root . '/_super_audit_backups/' . date('Ymd_His');
...
<title>Update Super Audit V2</title>
...
<div style="padding:16px 20px;background:#0f172a;color:#fff;font-weight:700">تحديث Godyar Super Audit V2</div>
<?php

if (!$RUN) {
    echo '<div style="padding:20px">';
    echo '<p>هذا السكربت سيقوم بتحديث ملف الفحص الذكي إلى النسخة V2.</p>';
    echo '<a href="?run=1" style="display:inline-block;background:#0f172a;color:#fff;padding:10px 16px;border-radius:10px;text-decoration:none">بدء التحديث</a>';
    echo '</div></div></body></html>';
    exit;
}

$target = $ROOT . '/admin/system/super_audit.php';
backup_file($target, $ROOT);
out('تم إنشاء نسخة احتياطية إن وُجد الملف القديم.', 'ok');
...
if (!write_file($target, $code)) {
    out('فشل في كتابة admin/system/super_audit.php', 'err');
    echo '</div></div></body></html>';
    exit;
}

out('تم تحديث admin/system/super_audit.php إلى النسخة V2.', 'ok');
out('شغّل الآن: /admin/system/super_audit.php', 'ok');
